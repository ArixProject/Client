...
$(document).ready(function () {
    const request = prompt("Server Ip below", "ws://127.0.0.1:443");
    connect(request)
});
var SKIN_URL = "./skins/";
...
function setRegion(a) {
    if (a && a != w) {
        ...
        w = window.localStorage.location = a;
        $(".region-message").hide();
...
function clientPacket(view, offset) {
    ...
    */
    ...
    if (Data.title) {
        $(document).prop('title', clientData.title);
        $("#titleh").text(clientData.title);
    }
    if (Data.defaultusername) $("#nick").val(clientData.defaultusername);
    if (Data.nickplaceholder) $("#nick").attr("placeholder", clientData.nickplaceholder)
    if (Data.instructions) $("#customins").text(clientData.instructions);
    if (Data.customHTML) $("#customht").html(clientData.customHTML);
    if (Data.maxName) $("#nick").attr("maxlength", clientData.maxName);
    ...
    if (Data.chat) {
        if (clientData.chat < 2) $("#chat_textbox").hide();
        else $("#chat_textbox").show();
    }
    ...
    if (clientData.chat == 0 || clientData.chat == 3) $('#cchat').attr('disabled', true);
    else $('#cchat').attr('disabled', false);
    if (clientData.darkBG == 0 || clientData.darkBG == 3) $('#cdark').attr('disabled', true);
    else $('#cdark').attr('disabled', false);
    if (clientData.skins == 0 || clientData.skins == 3) $('#cskin').attr('disabled', true);
    else $('#cskin').attr('disabled', false);
    if (clientData.grid == 0 || clientData.grid == 3) $('#cgrid').attr('disabled', true);
    else $('#cgrid').attr('disabled', false);
    f
    if (clientData.acid == 0 || clientData.acid == 3) $('#cacid').attr('disabled', true);
    else $('#cacid').attr('disabled', false);
    if (clientData.colors == 0 || clientData.colors == 3) $('#ccolor').attr('disabled', true);
    else $('#ccolor').attr('disabled', false);
    if (clientData.names == 0 || clientData.names == 3) $('#cname').attr('disabled', true);
    else $('#cname').attr('disabled', false);
    if (clientData.showMass == 0 || clientData.showMass == 3) $('#cmass').attr('disabled', true);
    else $('#cmass').attr('disabled', false);
    if (clientData.smooth == 0 || clientData.smooth == 3) $('#csmooth').attr('disabled', true);
    else $('#csmooth').attr('disabled', false);
...
window.setHideGrid = function (arg) {
...
window.setHideChat = function (arg) {
    hideChat = arg;
...
window.setGameMode = function (arg) {
    if (arg != gameMode) {
        gameMode = arg;
        showConnecting();
    }
...
if (null != window.localStorage) {
    if (null == window.localStorage.AB8) {
        window.localStorage.AB8 = ~~(100 * Math.random());
    }
    Ra = +window.localStorage.AB8;
    window.ABGroup = Ra;
}
/*$.get(localProtocol + "//gc.agar.io", function (a) {
 var b = a.split(" ");
 a = b[0];
 b = b[1] || "";
